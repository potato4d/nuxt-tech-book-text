...
ひとつ、特殊なコンポーネントとして、 `<nuxt />` があります。これは単純に、ここを slot とし、ページコンポーネントが挿入されるということを示すコンポーネントとなります。
レイアウトファイルではこの `<nuxt />` は必須となりますので、ページの中のコンテンツ本文に相当する場所に `<nuxt />` を記述しておくと良いでしょう。
...
最後に、開発が完了した頃にあと、本番環境のテストで気づいて必要となりがちな、エラーページのカスタマイズについてご紹介いたします。

### エラーページの規約

Nuxt のエラーページは、特殊なルールによって決定されます。
エラーページは、レイアウトファイルとして処理され、必ず `layouts/error.vue` に配置するルールとなっています。
また、エラーページはレイアウトファイルでありながら `<nuxt />` を持たず、ページコンポーネントと同じ振る舞いをします。

したがって、それ自体がレイアウトでありながら、何かしらのレイアウトを指定してその中にエラーページが追加される仕様となります。

試しに `error-wrap.vue` と `error.vue` を実際に作成して、動作を見てみましょう。

### エラーページの実装

まずは `error-wrap.vue` からです。
エラーページ用のレイアウトなので、特に何もせずそのまま `<nuxt />` を返してしまいましょう。
`layouts/error-wrap.vue` を以下のように記述してください。
...
その上で、エラーのレイアウトを作成しましょう。
`layouts/error.vue` を作成し、以下のように記述してください。
...
特徴的なのは、 `error` プロパティです。

この `error` プロパティは、エラーが発生した時のエラー情報が格納されており、例えば error.statusCode では、エラーの HTTP ステータスコードが返ります。
最も多いユースケースでは、 条件分岐で `error.statusCode == 404` などを追加してやると、 5xx 系のエラーなのかただの 404 なのかによって表示を切り分けることができます。

error プロパティは、 error のオブジェクト内にも多くのプロパティを抱えているため、必要に応じて読み出して活用すると良いでしょう。

これを実行すると、以下のように表示されます。

ここまで表示できたら、あとは必要に応じてデザインをカスタマイズするだけです。
